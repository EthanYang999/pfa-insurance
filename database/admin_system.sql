...
CREATE POLICY "Only admins can access system_logs" ON system_logs
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM admin_users 
            WHERE user_id = auth.uid() AND is_active = true
...
CREATE POLICY "Only admins can access system_config" ON system_config
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM admin_users 
            WHERE user_id = auth.uid() AND is_active = true
...
CREATE POLICY "Only admins can access service_health" ON service_health
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM admin_users 
            WHERE user_id = auth.uid() AND is_active = true
...
CREATE POLICY "Only admins can access user_activity_stats" ON user_activity_stats
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM admin_users 
            WHERE user_id = auth.uid() AND is_active = true
...
CREATE POLICY "Only admins can access api_call_stats" ON api_call_stats
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM admin_users 
            WHERE user_id = auth.uid() AND is_active = true
